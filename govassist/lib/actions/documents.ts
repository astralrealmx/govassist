...
import { DOCUMENT_TYPES, DocumentType, PROFILE_FIELD_MAP, EDUCATION_FIELD_MAP, DOCUMENT_TYPE_SLUGS } from "@/lib/document-types";
...
export async function applyExtractedFieldsToProfileAction(documentId: string): Promise<{ error?: string; success?: boolean; message?: string }> {
  ...
  const fieldMap = new Map(doc.extracted_fields.map((f: ExtractedField) => [f.label, f.value]));

  const profileUpdate: Record<string, string> = {};
  for (const [label, profileField] of Object.entries(PROFILE_FIELD_MAP)) {
    const value = fieldMap.get(label);
    if (value && profileField) profileUpdate[profileField] = value;
  }
